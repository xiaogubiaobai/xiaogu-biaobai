...
var 配置 = {
     ...
     * ⚠️ 它只管签到那一层 —— 切号该切还是切。界面上「切号签到(干跑)」那个按钮
     *    就是靠临时打开它来「真切号 + 假签到」的。
...
ui.layout(
    <vertical bg="#f2f2f5" h="*">
        ...
        */}
        ...
        <vertical>
        <vertical id="主卡" bg="#ffffff" margin="14 0" padding="18">
            ...
            */}
            ...
            {/* 只切号、不签到 —— 专门用来验证切号这条链路,
                免得每测一次就烧掉几个号当天的表白机会。*/}
            <button id="切号钮" text="开始切号(只切不签)" textSize="16sp" h="52"
...
ui.切签钮.on("click", function () {
    ...
    开跑("切号签到(干跑)", function () {
        /*
         * 真切号 + 假签到:整条流程都走一遍(开面板、切号、进每个角色页、读状态),
         * 唯独最后那一下表白不点。
         * ⚠️ 这个组合是刻意的:每个角色每个号每天只有一次表白机会,
         *    拿去调流程就没了。先用它把链路跑通,确认没问题再真签。
         */
        var 原 = 配置.干跑;
        配置.干跑 = true;
        try { 跑全部账号(跑一轮); }

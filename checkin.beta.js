...
function 开跑(任务名, 任务) {
    ...
    账号进度 = ""; 账号名显示 = "";
    抓过分身框 = false; 点过分身框 = 0; 说过认不出 = false; 说过始终提示 = false;
...
// ⚠️ 「始终/总是」那颗**永远不点** —— 它改的是系统里的持久设置,用户没让我们动。
//    (用户自己点是另一回事,日志里会提示他这条路。)
...
function 过分身框() {
    ...
    if (!要) {
        ...
        if (候.length < 2) {
             ...
             * vivo 的 com.vivo.appfilter「XX 想要打开 YY」,底下只有
             * 「始终打开 / 仅打开一次 / 取消」。那就找那颗「仅打开一次」按下去。
             * ⚠️ 绝不点「始终打开」—— 那是改系统里的持久设置,用户没让我们动。
             *    但要在日志里告诉他这条路:他自己点一次,以后就不再弹了。
             */
            var 钮 = 找确认钮(条);
            if (钮) {
                点过分身框++;
            ...
            if (!说过认不出) {
                ...
                记("    认不出这个框该点哪(没有 main/clone,也没找到「仅一次」那类按钮)"
                   + " —— 请自己点一下");
            }
            return false;
        }

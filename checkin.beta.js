...
function 开跑(任务名, 任务) {
    ...
    账号进度 = ""; 账号名显示 = "";
    抓过分身框 = false; 点过分身框 = 0;   // 见「厂商的应用分身选择框」那一节
...
/** 点这个节点。⚠️ 条目上的文字多半 clickable=false,要往上找可点的祖先;都没有就按坐标点 */
function 点节点(项) {
    var n = 项.点;
    for (var i = 0; i < 6 && n; i++) {
        var 可 = false;
...
function 过分身框() {
    ...
    if (候.length < 2) {
        if (点过分身框 === 0) {
            点过分身框 = 1;      // 只说一次
            记("    按「" + 名 + "」只认出 " + 候.length + " 条,不敢替你点 —— 请在框里自己选一下");
        }
        return false;
    }
    ...
    /*
     * 哪条是本尊?**文字最短**的那条。
     * ⚠️ 分身的显示名是在本尊名字上加东西(vivo 加「Ⅱ」),所以短的那条必是本尊。
     *    别去匹配「Ⅱ」这个具体字符 —— 换个厂商就不是它了。
     */
    var 短 = 候[0];
    for (var k = 1; k < 候.length; k++) if (候[k].文.length < 短.文.length) 短 = 候[k];
    var 要 = 短;
    if (腾讯user !== 我的user) {
        for (var m = 0; m < 候.length; m++) if (候[m] !== 短) { 要 = 候[m]; break; }
    }
    点过分身框++;
    记("    替你点「" + 要.文 + "」(这次要的是" + (腾讯user === 我的user ? "本机" : "分身") + ")");
    记("    结果:" + 点节点(要));
    sleep(1200);

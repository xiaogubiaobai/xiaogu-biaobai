...
function 开跑(任务名, 任务) {
    ...
    账号进度 = ""; 账号名显示 = "";
    抓过分身框 = false; 点过分身框 = 0; 说过认不出 = false;
...
var 抓过分身框 = false;    // 一轮只抓一次结构,别刷屏
var 点过分身框 = 0;        // 这一轮替用户点了几次(vivo 是每发一次深链弹一次,十几次很正常)
var 说过认不出 = false;    // 「认不出哪条是哪个」只说一次
 ...
 */
var 只此一次词 = ["仅此一次", "只此一次", "仅一次", "回のみ", "just once", "only once"];
var 始终词 = ["始终", "总是", "常時", "always", "永远"];
...
/**
 * 按坐标点。⚠️ 派发手势之前把悬浮条设成不接收触摸,免得点到自己。
 */
function 坐标点(项) {
    try {
        try { if (控制条) 控制条.setTouchable(false); } catch (e) {}
        var 成 = click(项.框.centerX(), 项.框.centerY());
        try { if (控制条) 控制条.setTouchable(true); } catch (e) {}
        return !!成;
    } catch (e) { return false; }
}

/** 无障碍点(节点自己或最近的可点祖先)。返回的只是「派发出去了」,**不代表有效** */
function 无障碍点(项) {
    var n = 项.点;
    for (var i = 0; i < 6 && n; i++) {
        var 可 = false;
 ...
 * 点框里的一条,并且**验证它真的被点掉了**。
 *
 * ⚠️⚠️ 这里踩过一个大跟头:原先先试 `performAction(ACTION_CLICK)`,成功就返回
 *    「无障碍点击」—— 但那个返回值只代表**动作派发出去了**,不代表那个 View 响应。
 *    vivo 这个框的行是自己处理触摸事件的,无障碍点击对它**没用**:
 *    日志一片「无障碍点击」,实际上每次都是用户自己手点掉的,我还以为修好了。
 *    (2026-09-17 用户纠正:「其实是每弹一次用户都要自己手动选一次」。)
 *
 * 所以改成:**先按坐标点**(跟人手一样)→ 验框还在不在 → 还在就再试无障碍 → 再验。
 * 并且日志只报**验证过的结果**,不再报「派发成功」。
 */
function 点掉框里的(项) {
    if (坐标点(项) && 等框消失(1500)) return "坐标点掉了";
    if (无障碍点(项) && 等框消失(1500)) return "无障碍点掉了";
    // 有的机型条目本身不吃点击,得点它里面那个文字
    if (项.文子 && 坐标点(项.文子) && 等框消失(1500)) return "点文字点掉了";
    return "点了但框还在";
}
...
function 过分身框() {
    ...
    if (!要) {
        ...
        for (var j = 0; j < 条.length; j++)
            if (名 && 条[j].文 && 条[j].文.indexOf(名) >= 0) 候.push(条[j]);
        if (候.length < 2) {
            if (!说过认不出) {
                说过认不出 = true;      // 只说一次,别每次弹框都刷一遍
                记("    认不出哪条是哪个(按 id 没找到 main/clone,按「" + 名 + "」只有 "
                   + 候.length + " 条)—— 请在框里自己选一下");
            }
            return false;
        }
        var 短 = 候[0];
